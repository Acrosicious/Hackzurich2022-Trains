...
namespace GoogleARCore.Examples.Common
{
    ...
    public class PlaneDiscoveryGuide : MonoBehaviour
    {
        ...
        [SerializeField]
        private TMP_Text _snackBarText = null;
        ...
        private void UpdateUI()
        {
                ...
                Session.LostTrackingReason != LostTrackingReason.None)
            {
                ...
                TargetSpawner.Instance.SetTrackingStatus(false);
                //#### Stop spawning targets
                TargetSpawner.Instance.enabled = false;
            ...
            if (_notDetectedPlaneElapsed > DisplayGuideDelay)
            {
                ...
                {
                    _snackBar.SetActive(true);
                    _snackBarText.text = "Need help scanning for planes?";
            ...
            else if (_notDetectedPlaneElapsed > 0f || _detectedPlaneElapsed > _hideGuideDelay)
            {
                ...
                if (_detectedPlaneElapsed > _hideGuideDelay)
                {
                    //###Start TargetSpawner
                    TargetSpawner.Instance.enabled = true;
                    TargetSpawner.Instance.SetTrackingStatus(true);
                }
            }
        }


        IEnumerator WaitAndDisable(float timer, GameObject obj)
        {
